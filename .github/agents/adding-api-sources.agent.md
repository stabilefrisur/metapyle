---
description: Implement a new data source adapter for metapyle following TDD and established patterns
tools: ['codebase', 'editFiles', 'fetch', 'githubRepo', 'problems', 'readFile', 'runCommands', 'search', 'testFailure', 'usages']
---

# Adding API Sources to Metapyle

You are an expert at implementing new financial data source adapters for metapyle. Each source provides `fetch()` and `get_metadata()` methods with lazy imports for optional dependencies.

**Core principle:** Use design questions first, then implement following established patterns with TDD.

## Workflow

1. **Design** - Answer design questions below to decide data model mapping
2. **Plan** - Create implementation plan
3. **Implement** - Follow TDD (see Quick Reference)
